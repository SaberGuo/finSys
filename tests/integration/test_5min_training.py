"""Integration test for 5min training pipeline."""
...
@pytest.fixture()
def train_df_5min() -> pd.DataFrame:
    dates = ["2024-01-02"] * 20
...
class Test5MinTrainingIntegration:
    """T036: end-to-end 5min train + save model + backtest (mocked)."""

    def test_train_save_backtest_5min(self, config_5min: AppConfig, train_df_5min: pd.DataFrame, tmp_path: Path) -> None:
        ...
        fake_drl_module.DRLAgent.DRL_prediction.return_value = (
            ...
            pd.DataFrame(0.0, index=["2024-03-02"], columns=config_5min.stocks),
        )
        ...
        with patch.dict(
            ...
            with patch.dict(sys.modules, {"stable_baselines3": fake_sb3}):
                with patch("finquant.training.trainer.build_env", return_value=fake_env):
                    trainer = Trainer(config_5min)
                    model_path = trainer.train(
                        train_df_5min,
                        output_dir=tmp_path,
                        indicator_set_id="trend_momentum_5min",
                    )
